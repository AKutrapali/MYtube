...
import { hotKeys } from './hot_keys';

let data;

export async function loadFullVideoPart(){
  ...
  videoContainer.addEventListener('click', () => {playVideo(data, event.target.parentNode)});
...
document.addEventListener('keydown', (event) => {hotKeys(event, data)});
